...
class RandomFlip:
    ...
    def __init__(self, random_state, axes=None, axis_prob=0.5, **kwargs):
        assert random_state is not None, 'RandomState cannot be None'
        self.random_state = random_state
        self.axes = (0, 1, 2) if axes is None else axes
...
class ResizeCrop:
    def __init__(self, random_state=None, inter_size=(286, 286), target_size=(256, 256), is_label=False, **kwargs):
        self.random_state = random_state
        self.inter_size = inter_size
        self.target_size = target_size
        self.interpolation = cv2.INTER_CUBIC if is_label else cv2.INTER_NEAREST
    ...
    def __call__(self, m):
        ...
        w_offset = self.random_state.randint(0, max(0, self.inter_size[1] - self.target_size[1] - 1))
        h_offset = self.random_state.randint(0, max(0, self.inter_size[0] - self.target_size[0] - 1))
        if m.ndim == 3:
            out = []
            for idx, slice in enumerate(m):
                slice = cv2.resize(slice, self.inter_size, interpolation=self.interpolation)
                slice = slice[w_offset:w_offset+self.target_size[1], h_offset:h_offset+self.target_size[0]]
                out.append(slice)
            ...
            for c in m:
                out = []
                for idx, slice in enumerate(c):
                    slice = cv2.resize(slice, self.inter_size, interpolation=self.interpolation)
                    slice = slice[w_offset:w_offset+self.target_size[1], h_offset:h_offset+self.target_size[0]]
                    out.append(slice)
                channels.append(np.stack(out, axis=0))
            return np.stack(channels, axis=0)


class CropToFixed:
    def __init__(self, random_state=None, size=(256, 256), centered=False, mode='reflect', **kwargs):
        self.random_state = random_state
    ...
    def __call__(self, m):
        ...
        if m.ndim == 3:
            ...
            return np.pad(result, pad_width=((0, 0), y_pad, x_pad), mode=self.mode)
        else:
            channels = []
            for c in range(m.shape[0]):
                result = m[c][:, y_start:y_start + self.crop_y, x_start:x_start + self.crop_x]
                channels.append(np.pad(result, pad_width=((0, 0), y_pad, x_pad), mode=self.mode))
            return np.stack(channels, axis=0)
    ...
    def __call__(self, m):
        if self.mean is not None:
            ...
            if self.channelwise:
                ...
                std = np.clip(std, a_min=self.eps, a_max=None)
            else:
                mean = np.mean(m)
                std = np.std(m)
...
class RepeatDim:
    def __init__(self, repeat_dim=0, **kwargs):
        self.repeat_dim = repeat_dim

    def __call__(self, m):
        if isinstance(m, torch.Tensor):
            m = torch.repeat_interleave(m, 3, dim=self.repeat_dim)
        else:
            m = np.repeat(m, 3, axis=self.repeat_dim)
...
class ToTensor:
    ...
    def __init__(self, expand_dims, dtype='float32', **kwargs):
...
class Transformer:
    ...
    def extra_raw_transform(self):
        return self._create_transform('extra_raw', False)
    ...
    def _transformer_class(class_name):
        m = importlib.import_module('data.augment.transforms')
        clazz = getattr(m, class_name)
        return clazz

    def _create_transform(self, name, keep_seed=True):
        assert name in self.phase_config, f'Could not find {name} transform'
        return Compose([
            self._create_augmentation(c, keep_seed) for c in self.phase_config[name].items()
            if c[1]['enabled']
        ])

    def _create_augmentation(self, c, keep_seed=True):
        ...
        if keep_seed:
            ...
            config['random_state'] = np.random.RandomState(np.random.randint(10000000))
