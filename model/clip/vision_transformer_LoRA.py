...
class LoRA(nn.Module):
    def __init__(self, dim, r=4, alpha=1.0):
        super().__init__()
        self.r = r
        self.alpha = alpha
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., use_lora=False, lora_rank=4, lora_alpha=1.0):
        super().__init__()
        ...
        if use_lora:
            self.lora_q = LoRA(dim, r=lora_rank, alpha=lora_alpha)
            # self.lora_k = LoRA(dim, r=lora_rank, alpha=lora_alpha)
            self.lora_v = LoRA(dim, r=lora_rank, alpha=lora_alpha)

    def forward(self, x):
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)   # [B,num_heads,N,C//num_heads] make torchscript happy (cannot use tensor as tuple)

        if self.use_lora:
            q = q + self.lora_q(x).reshape(B,N,self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            # k = k + self.lora_k(k).reshape(B,N,self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
            v = v + self.lora_v(x).reshape(B,N,self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
...
class Block(nn.Module):
    ...
    def __init__(
        ...
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, use_lora=use_lora, lora_rank=lora_rank, lora_alpha=lora_alpha)
        ...
        if self.add_prompt:
            self.n_ctx_visual = vision_ctx  # hyperparameter
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.VPT_shallow = use_vpt 
        if self.VPT_shallow:
            # Add visual prompt tokens here
            n_ctx = vision_ctx  # hyperparameter
        ...
        self.blocks = nn.Sequential(*[
            block_fn(
                ...
                act_layer=act_layer,
                add_prompt = True if (use_vpt and i!=0) else False,
