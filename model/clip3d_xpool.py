...
class CLIPTransformer(nn.Module):
    def __init__(self, clip_backbone, freeze_backbone, embed_dim=512, num_mha_heads=1, transformer_dropout=0.3, prompt_length=0, vision_ctx=0, out_cls=1):
    ...
    def forward(self, text, img):
        ...
        text_embeds = self.clip_backbone.encode_text(text) #[2B,E] -> [2B,E]
        ...
        video_features_pooled, attn_weights = self.pool_slices(text_embeds_unCT, img_embeds)
        
        out_aggregated = torch.diagonal(video_features_pooled,dim1=0,dim2=1).t() + text_embeds_unCT
        out = self.tail(out_aggregated)
        return out, video_features_pooled, attn_weights, text_embeds_CT, img_embeds
