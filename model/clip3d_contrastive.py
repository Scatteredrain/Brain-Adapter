...
# from .clip.vision_transformer import vit_base_patch16_224
import numpy as np
...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds):
        ...
        attention_logits = k @ q #num_vids x num_heads x num_frames x head_dim @ num_texts x num_heads x head_dim x 1
        attention_logits = attention_logits / math.sqrt(self.head_dim)
        attention_weights = F.softmax(attention_logits, dim=2)

        # num_vids x num_heads x head_dim x 1
        attention = v @ attention_weights #num_vids x num_heads x head_dim x num_frames @ num_vids(num_texts) x num_heads x num_frames x 1
        # num_vids x 1 x num_heads x head_dim
        attention = attention.permute(0,3,1,2)
...
class CLIPTransformer(nn.Module):
    def __init__(self, clip_backbone, freeze_backbone, freeze_text_backbone, embed_dim=512, num_mha_heads=1, transformer_dropout=0.3, 
        prompt_length=0, use_vpt=False, vision_ctx=0, use_lora=False, lora_rank=4, lora_alpha=1, out_cls=2, add_reg_head=False, add_cls_head=False,add_retrieval_reg_head=False):
        super(CLIPTransformer, self).__init__()
        self.clip_backbone = clip_backbone
        vit = vit_base_patch16_224(False, prompt_depth=prompt_length,
        use_vpt=use_vpt, vision_ctx=vision_ctx, use_lora=use_lora, lora_rank=lora_rank, lora_alpha=lora_alpha)
        # vit = vit_base_patch16_224(False, prompt_depth=prompt_length,vision_ctx=vision_ctx)
    ...
    def forward(self, text_basic, text_ct, img):
        ...
        text_embeds_CT = self.clip_backbone.encode_text(text_ct)

        img_embeds = self.clip_backbone.encode_image(img) 
        img_embeds = img_embeds.reshape(bs, num_slices, -1) #[B,T,E]
        # out_aggregated: num_vids x [1 or num_texts] x embed_dim
        # attn_weights: num_vids x 1 x num_frames x [1 or num_texts]
        out_aggregated, attn_weights = self.pool_slices(text_embeds_CT, img_embeds)

        out_aggregated = out_aggregated.squeeze(1) #[B,E] or [B,B,E]

        return out_aggregated, attn_weights, text_embeds_CT, img_embeds